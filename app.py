...
from utils.auto_import import check_and_perform_auto_import, display_auto_import_status
...
pg = st.navigation([
    ...
    st.Page("pages/1_📤_Upload_Data.py", title="Upload Data", icon="📤"),
    st.Page("pages/2_📊_Dashboard.py", title="Dashboard", icon="📊"),
    st.Page("pages/3_🤖_AI_Assistant.py", title="AI Assistant", icon="🤖"),
    st.Page("pages/4_📋_Doctor_Report.py", title="Doctor Report", icon="📋"),
])

# --- SIDEBAR: CONTENT (Displayed on all pages) ---
with st.sidebar:
    st.markdown("### 📥 Auto-Import")
    check_and_perform_auto_import()
    display_auto_import_status()
    st.divider()
